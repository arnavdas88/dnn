...
extern "C" __declspec(dllexport) void WINAPI CTCComputeAlphas(
	...
	float* pa)
{
	std::fill(pa, pa + (gsl::narrow_cast<ptrdiff_t>(S) * T), -INFINITY);
...
extern "C" __declspec(dllexport) void WINAPI CTCComputeBetas(int T, int A, int S, const float* py, const int* labels, float* pb)
{
	std::fill(pb, pb + (gsl::narrow_cast<ptrdiff_t>(S) * T), -INFINITY);

	int start = __max(0, S - 2);
	int end = __min(2 * T, S);

	pb += gsl::narrow_cast<ptrdiff_t>(S) * (gsl::narrow_cast<ptrdiff_t>(T) - 1);
	py += gsl::narrow_cast<ptrdiff_t>(A) * (gsl::narrow_cast<ptrdiff_t>(T) - 1);
...
extern "C" __declspec(dllexport) void WINAPI CTCReduceAlphasBetas(int T, int A, int S, const float* pab, const int* labels, float* pdy)
{
	std::fill(pdy, pdy + (gsl::narrow_cast<ptrdiff_t>(A) * T), -INFINITY);
