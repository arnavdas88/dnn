	...
	BOOL rowmajor)
{
	...
	if (!forward)
	{
		...
		ptrdiff_t tstart = gsl::narrow_cast<ptrdiff_t>(steps) - 1;
		g += tstart * glen;
		s += tstart * ylen;
		y += tstart * ylen;
	}
	...
	for (int t = 0; t < steps; t++, g += ginc, s += yinc, y += yinc)
	{
		...
		if (t == 0)
		{
			...
			{
				const float igate = g[i] = sigmoid(g[i]);
				const float jgate = jg[i] = ::tanhf(jg[i]);
				const float fgate = fg[i] = sigmoid(fg[i] + forgetBias);
				const float ogate = og[i] = sigmoid(og[i]);
	...
	for (int t = steps - 1; t >= 0; t--, g -= ginc, dg -= ginc, s -= yinc, ds -= yinc, y -= yinc, dy -= yinc)
	{
		...
		if (t == 0)
		{
			for (int i = 0; i < ylen; i++)
			{
				...
				// output gate
				dog[i] = dyi * sai * sigmoid_derivative2(ogate);

				// cell state gradient
				ds[i] += dyi * ogate * tanh_derivative2(sai);
				const float dsi = ds[i];

				// input gate
				dg[i] = dsi * jgate * sigmoid_derivative2(igate);

				// new input gate
				djg[i] = dsi * igate * tanh_derivative2(jgate);
		...
		else
		{
			...
			for (int i = 0; i < ylen; i++)
			{
				...
				// forget gate
				dfg[i] = dsi * sprev[i] * sigmoid_derivative2(fgate);
	...
	for (int t = steps - 1; t >= 0; t--, y -= yinc, dy -= yinc, g -= ginc, dg -= ginc)
	{
		...
		if (t == 0)
		{
			for (int i = 0; i < ylen; i++)
			{
				...
				// update gate
				dg[i] = dyi * candidate * sigmoid_derivative2(ugate);

				// reset gate
				drg[i] = 0;

				// candidate
				dcg[i] = dyi * ugate * tanh_derivative2(candidate);
			}
		}
		else
		{
			...
			for (int i = 0; i < ylen; i++)
			{
				...
				// update gate
				dg[i] = dyi * (candidate - state[i]) * sigmoid_derivative2(ugate);
			...
			for (int i = 0; i < ylen; i++)
			{
				...
				// reset gate
				drg[i] = drg[i] * state[i] * sigmoid_derivative2(rg[i]);
			}
