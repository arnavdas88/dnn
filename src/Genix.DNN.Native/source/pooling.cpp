	...
	const int* ystrides)
{
	...
	if (ksize1 == 2 && ksize2 == 2)
	{
		...
		_parallel_for(0, y0, 0, y1, [&](int ixy0, int iy1) {
			...
			const float* xww = xw + (ptrdiff_t(ixy0) * xstride0) + (ptrdiff_t(ix1) * xstride1);
			...
			if (size1 > 0)
			{
				...
				const float* xww2 = xww1 != NULL ? xww1 : xww;
				for (int iy2 = 0, ix2 = -kpadding2; iy2 < y2; iy2++, ix2 += kstride2, yww += ystride2, xww2 += xstride2K)
				{
					const int ix2e = min(ix2 + ksize2, x2);

					int size2 = ix2e - max(ix2, 0);
					switch (size2)
					{
					case 0:
	...
	const int* ystrides)
{
	...
	// cycle by x, so threads to not write to the same memory
	for (int ixy0 = 0; ixy0 < y0; ixy0++) {
		for (int ix1 = 0; ix1 < x1; ix1++) {
			//_parallel_for(0, y0, 0, x1, [&](int ixy0, int ix1) {

			if (kstride1 > ksize1 && ((ix1 + kpadding1) % kstride1) >= ksize1)
			{
				continue;
			}
			...
			const int iy1b = max((ix1 + kpadding1 - (ksize1 - 1)) / kstride1, 0);
			const int iy1e = min(((ix1 + kpadding1) / kstride1) + 1, y1);

			float* dxww = dxw + (ptrdiff_t(ixy0) * xstride0) + (ptrdiff_t(ix1) * xstride1);
			const float* dyww = dyw + (ptrdiff_t(ixy0) * ystride0) + (ptrdiff_t(iy1b) * ystride1);

			for (int ix2 = 0; ix2 < x2; ix2++, dxww += xstride2)
			{
				if (kstride2 > ksize2 && ((ix2 + kpadding2) % kstride2) >= ksize2)
				{
					continue;
				}

				const int iy2b = max((ix2 + kpadding2 - (ksize2 - 1)) / kstride2, 0);
				const int iy2e = min(((ix2 + kpadding2) / kstride2) + 1, y2);

				const float* dyww1 = dyww + (ptrdiff_t(iy2b) * ystride2);
				for (int iy1 = iy1b; iy1 < iy1e; iy1++, dyww1 += ystride1)
				{
					const float* dyww2 = dyww1;
					for (int iy2 = iy2b; iy2 < iy2e; iy2++, dyww2 += ystride2)
					{
						add_productc(ystride2, dyww2, alpha, dxww);
					}
				}
			}
			/*});*/
